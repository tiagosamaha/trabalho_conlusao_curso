...
O \textit{jQuery} é uma popular biblioteca \textit{cross-browser} que facilita a implementação de diversas funcionalidades no \textit{client-side}, como manipulação de elementos, interações \textit{AJAX} e tratamento de eventos.

\section{Amberjack}
\label{sec:amberjack}

O \textit{Amberjack2} é a segunda versão da biblioteca \textit{Amberjack} (http://amberjack.org/), o objetivo original desta ferramenta é a criação de \textit{tours} para sites, guiando o usuário através das páginas, exibindo pequenos textos descritivos enquanto aplica um efeito de realce no elemento em foco no contexto. Um exemplo do \textit{tour} gerado por esta biblioteca pode ser visto na Figura \ref{figura_22}.
