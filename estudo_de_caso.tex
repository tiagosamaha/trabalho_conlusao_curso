...
A vantagem de utilizar este formato é que ele pode ser facilmente entendível por humanos e ao mesmo tempo é flexível e facilmente manipulável, conforme descrito no tópico \ref{sec:yaml}.

Usando esse formato de entrada, o cadastro de um novo cenário pode ser descrito conforme exemplificado no Código \ref{lst:codigo38}.
...
Por fim, para implementar esta arquitetura integrada a Biblioteca Digital, o \textit{framework} utilizado disponobiliza um conjunto de \textit{scripts} que auxiliam o desenvolvedor. O Código \ref{lst:codigo42} representa o comando que gera esta estrutura.
...
Após isto, a classe responsável pela descrição do cenário e toda a interface de gerenciamento pelo usuário é criada. Esta classe irá conter toda a lógica de leitura do cenário escrito no formato YAML.

\subsection{Funcionalidades desenvolvidas}

Para a implementação do processo da execução do cenário, foi utilizado a linguagem \textit{JavaScript}, através das bibliotecas \textit{open-source: jQuery} (tópico \ref{sec:jquery}) e \textit{Amberjack2} (tópico \ref{sec:amberjack}).

Durante o processo de desenvolvimento foi detectado um problema causado pela necessidade da passagem de parâmetros pela \textit{URL}, obrigatórios para que a biblioteca \textit{Amberjack} fosse corretamente inicializada. Esses parâmetros são facilmente repassados quando a troca de página se da por uma simples requisição \textit{GET} \cite{W3}, entretanto quando ela é resultante de redirecionamento vindo do servidor, esses parâmetros se perdem.

Uma solução simples e eficaz para este problema foi o uso de \textit{cookies}. \textit{Cookies} são pequenas quantidades de informação que uma aplicação \textit{web} pode salvar no navegador, com o objetivo de recuperar posteriormente. 

A seguir, são descritas as funcionalidades especificadas para a ferramenta NSI-BD-Tour.

\textbf{F01.} Cadastro e gerenciamento de cenários usando \textit{YAML}

O objetivo dessa funcionalidade é permitir o usuário descrever os cenários em uma linguagem simples - formato \textit{YAML}. O cenário descrito tem será utilizado para validar e guiar o usuário no emprego do sistema. Na implementação dessa funcionalidade, foi criado um cadastro de cenários, onde há um campo de texto em que o usuário pode descrever o cenário.
...
\textbf{F02.} Análise e extração de dados do \textit{YAML}

O objetivo da análise e extração de dados do \textit{YAML} é obter os atributos do cenário, necessários para sua renderização no navegador.

A biblioteca usada para o processo de extração foi a \textit{Psych} \cite{PSYCH}. Essa biblioteca fornece uma maneira fácil de se transformar um texto em formato \textit{YAML} para uma estrutura nativa do \textit{Ruby} composta por dicionários, listas e valores simples. Um exemplo do uso da \textit{Pysch} pode ser visto no Código \ref{lst:codigo39}.

{\singlespace
\begin{lstlisting}[caption=Exemplo de utilização da bibliteca em \textit{Psych},label={lst:codigo39}]
require "psych"

yaml = <<-YAML
  Livro:
    titulo: 'Poignant Guide to Ruby'
    autor: '_why'
    tags:
      - awesome
      - ruby
...
# => {"Livro"=>{"titulo"=>"Poignant Guide to Ruby", "autor"=>"_why", "tags"=>["awesome", "ruby", "book"]}}
\end{lstlisting}
}

Portanto, o NSI-BD-Tour necessita de um método (Código \ref{lst:codigo37}, linhas 6-14) responsável pela conversão dos cenários descritos em YAML para utilizá-los internamente na classe \textit{Scenario}.

\textbf{F03.} Renderização do cenário

A renderização do cenário tem como objetivo fornecer todos os dados necessários para que a biblioteca \textit{Amberjack2} possa criar e conduzir os cenários pré-cadastrados.
Quando um usuário iniciar a execução de um cenário, seus atributos devem ser inseridos como código \textit{HTML} da página. Este código tem sua visualização oculta para usuário e é de uso interno da biblioteca. Este deve obedecer a um formato pré-especificado pelo \textit{Amberjack2}, como pode ser visto no Código \ref{lst:codigo40}.
...
Este cenário descrito em HTML fica oculto conforme a presença do parâmetro (atributo id apresentado na linha 1 do Código \ref{lst:codigo40}) de chamada do Amberjack2 ou a exitência do \textit{cookie} que identifica o cenário atual.

\textbf{F04.} Gerenciar cenário em execução usando cookies

O uso de \textit{cookies} foi necessário para contornar a necessidade da biblioteca \textit{Amberjack} de ter sempre os parâmetros de identificação do cenário na \textit{URL} corrente.
A solução para este problema foi extender o \textit{Amberjack2}, adicionando o comportamento de manipular \textit{cookies}. Desta maneira, depois da primeira requisição \textit{GET} responsável por iniciar a execução de um cenário, os dados necessários pelo \textit{Amberjack2}, como o identificador do cenário e o tema visual escolhido, ficam persistidos no navegador, eliminando a necessidade da passagem de quaisquer parâmetros relativos ao cenário executado.
